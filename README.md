...
Portable describes how the application is distributed; interview data is deliberately stored outside the extracted program folder. Persistent data lives under the Tauri application-data directory returned by `app.path().app_data_dir()` — for this bundle identifier (`com.ai-interviewer.app`) that is `%APPDATA%\com.ai-interviewer.app` on Windows. This keeps data across application upgrades and prevents it from being mixed with packaged tools.
